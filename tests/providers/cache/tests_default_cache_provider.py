...
def test_persist_cache_on_database_not_exist(shelve_mock):
...
@mock.patch('shelve.open', return_value={})
def test_persist_cache_on_database_exist(shelve_mock):
...
@mock.patch('shelve.open', return_value={})
def test_persist_not_duplicate_ip_address(shelve_mock):
    """Must be return country if address is cached on existing database."""
    first_data = {'country': 'AU', 'ip': '1.1.1.1'}
...
def test_persist_get_address(shelve_mock, get_request_mock):
    ...
    req = cache_provider.get(get_request_mock)
